...
class Recipe(db.Model):
    ...
    def get_all_by_user(cls, user_id,page,per_page, visibility = 'public'):
        if visibility =='public':
            return cls.query.filter_by(user_id = user_id, is_publish=True).order_by(cls.created_at.desc()).paginate(page=page, per_page=per_page)
    ...
    def get_all_published(cls,q,page,per_page):
        ...
        return cls.query.filter(or_(cls.name.ilike(keyword),
                ...
                order_by(desc(cls.created_at)).paginate(page=page,
                per_page=per_page)
